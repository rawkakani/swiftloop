import React from 'react';
import TaskManagement from './TaskManagement'; // Adjust the import path based on your file structure

function Dummy() {
  // Dummy user data
  const dummyUser = {
    userId: 'user123',
    userName: 'John Doe',
    userRole: 'Developer',
  };

  // Dummy task data
  const dummyTask = {
    teamId: 'team123',
    taskName: 'Dummy Task',
    dateTime: '2024-02-02T12:00:00',
    priority: 'High',
    assignedTo: 'John Doe',
    user: dummyUser,
  };

  return (
    <TaskManagement
      teamId={dummyTask.teamId}
      taskName={dummyTask.taskName}
      dateTime={dummyTask.dateTime}
      priority={dummyTask.priority}
      assignedTo={dummyTask.assignedTo}
      user={dummyTask.user}
    />
  );
}

export default Dummy;




 
<div className="div-42">
<div className="div-43">
  <div className="column-3">
    <div className="div-44">
      <div className="div-45">
        <div className="div-46">Backlog</div>
        <div className="div-47">
          <img
            loading="lazy"
            src="https://cdn.builder.io/api/v1/image/assets/TEMP/397670acc274a35abbdce1c95cebab78fccac6b31dfc024632ebf3788e687dd0?"
            className="img-13"
            alt = ""
          />
          <img
            loading="lazy"
            src="https://cdn.builder.io/api/v1/image/assets/TEMP/248d9e24c5c026a39aad4f37f34b1ec6c58a28c1263e3ea9bda2c06c438c8a4d?"
            className="img-14"
            alt = ""
          />
        </div>
      </div>
      <div className="div-48">
        <div className="div-49">
          <div className="div-50">
            <div className="div-51">Backend config</div>
            <div className="div-52">
              <img
                loading="lazy"
                src="https://cdn.builder.io/api/v1/image/assets/TEMP/687f48c4aabd60ebf5bb8710157b9991706d8c2bd1773f35197912770360223a?"
                className="img-15"
                alt = ""
              />
            </div>
          </div>
          <p className="sample-text">
            As a team license owner, I want to use multiplied
            limits
          </p>
          <div className="div-53">
            <div className="div-54">
              <img
                loading="lazy"
                src="https://cdn.builder.io/api/v1/image/assets/TEMP/59a87043a64c1a19fc05196705727b4165ed40128e2196c7836a97d72fbb08f7?"
                className="img-17"
                alt = ""
              />
              <div className="div-55">21/03/22</div>
            </div>
            <div className="div-56">D-18</div>
          </div>
          <div className="div-57">
            <img
              loading="lazy"
              srcSet="..."
              alt = ""
              className="img-18"
            />
            <div className="div-58">
              <img
                loading="lazy"
                src="https://cdn.builder.io/api/v1/image/assets/TEMP/8e08ac811a5ff415fec8d93ec4f3aa879b040c1f410e139bdb743534532af8b5?"
                className="img-19"
                alt = ""
              />
              <div className="div-59">12 comments</div>
              <img
                loading="lazy"
                src="https://cdn.builder.io/api/v1/image/assets/TEMP/6ff1645f58ebf13109177cd47f8cab10cc2e685346d7308b162277b2c632d668?"
                className="img-20"
                alt = ""
              />
              <div className="div-60">0 files</div>
            </div>
          </div>
        </div>
        <div className="div-61">
          <div className="div-62">
            <div className="div-63">
              Improve cards readability
            </div>
            <div className="div-64">
              <img
                loading="lazy"
                src="https://cdn.builder.io/api/v1/image/assets/TEMP/687f48c4aabd60ebf5bb8710157b9991706d8c2bd1773f35197912770360223a?"
                className="img-21"
                alt = ""
              />
            </div>
          </div>
          <div className="div-65">
            As a team license owner, I want to use multiplied
            limits
          </div>
          <div className="div-66">
            <div className="div-67">
              <img
                loading="lazy"
                src="https://cdn.builder.io/api/v1/image/assets/TEMP/0cf183460e563addb706582001b3d457613eec1c0f04b9605da7b761288a9c65?"
                className="img-22"
                alt = ""
              />
              <div className="div-68">21/03/22</div>
            </div>
            <div className="div-69">D-18</div>
          </div>
          <div className="div-70">
            <div className="div-71">
              <img
                loading="lazy"
                src="https://cdn.builder.io/api/v1/image/assets/TEMP/8e08ac811a5ff415fec8d93ec4f3aa879b040c1f410e139bdb743534532af8b5?"
                className="img-23"
                alt = ""
              />
              <div className="div-72">12 comments</div>
              <img
                loading="lazy"
                src="https://cdn.builder.io/api/v1/image/assets/TEMP/6ff1645f58ebf13109177cd47f8cab10cc2e685346d7308b162277b2c632d668?"
                className="img-24"
                alt = ""
              />
              <div className="div-73">0 files</div>
            </div>
            <img
              loading="lazy"
              srcSet="https://images.unsplash.com/photo-1543157145-f78c636d023d?q=80&w=2069&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
              className="img-25"
              alt = ""
            />
          </div>
        </div>
        <div className="div-74">
          <div className="div-75">
            <div className="div-76">
              <div className="div-77">D-3</div>
              <div className="div-78">Update tokens</div>
            </div>
            <div className="div-79">
              <img
                loading="lazy"
                src="https://images.unsplash.com/photo-1554151228-14d9def656e4?q=80&w=1886&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
                className="img-26"
                alt = ""
              />
            </div>
          </div>
          <img
            loading="lazy"
            srcSet="..."
            alt = ""
            className="img-27"
          />
        </div>
      </div>
    </div>
  </div>
  <div className="column-4">
    <div className="div-80">
      <div className="div-81">
        <div className="div-82">Todo</div>
        <div className="div-83">
          <img
            loading="lazy"
            src="https://cdn.builder.io/api/v1/image/assets/TEMP/397670acc274a35abbdce1c95cebab78fccac6b31dfc024632ebf3788e687dd0?"
            className="img-28"
            alt = ""
          />
          <img
            loading="lazy"
            src="https://cdn.builder.io/api/v1/image/assets/TEMP/248d9e24c5c026a39aad4f37f34b1ec6c58a28c1263e3ea9bda2c06c438c8a4d?"
            className="img-29"
            alt = ""
          />
        </div>
      </div>
      <div className="div-84">
        <div className="div-85">
          <div className="div-86">
            Improve cards readability
          </div>
          <div className="div-87">
            <img
              loading="lazy"
              src="https://cdn.builder.io/api/v1/image/assets/TEMP/687f48c4aabd60ebf5bb8710157b9991706d8c2bd1773f35197912770360223a?"
              className="img-30"
              alt = ""
            />
          </div>
        </div>
        <p className="sample-text">
          As a team license owner, I want to use multiplied
          limits
        </p>
        <div className="div-88">
          <div className="div-89">
            <img
              loading="lazy"
              src="https://cdn.builder.io/api/v1/image/assets/TEMP/59a87043a64c1a19fc05196705727b4165ed40128e2196c7836a97d72fbb08f7?"
              className="img-32"
              alt = ""
            />
            <div className="div-90">21/03/22</div>
          </div>
          <div className="div-91">D-18</div>
        </div>
        <div className="div-92">
          <img
            loading="lazy"
            srcSet="..."
            alt = ""
            className="img-33"
          />
          <div className="div-93">
            <img
              loading="lazy"
              src="https://cdn.builder.io/api/v1/image/assets/TEMP/8e08ac811a5ff415fec8d93ec4f3aa879b040c1f410e139bdb743534532af8b5?"
              className="img-34"
              alt = ""
            />
            <div className="div-94">12 comments</div>
            <img
              loading="lazy"
              src="https://cdn.builder.io/api/v1/image/assets/TEMP/6ff1645f58ebf13109177cd47f8cab10cc2e685346d7308b162277b2c632d668?"
              className="img-35"
              alt = ""
            />
            <div className="div-95">0 files</div>
          </div>
        </div>
      </div>
      <div className="div-96">
        <div className="div-97">
          <div className="div-98">
            Improve cards readability
          </div>
          <div className="div-99">
            <img
              loading="lazy"
              src="https://cdn.builder.io/api/v1/image/assets/TEMP/687f48c4aabd60ebf5bb8710157b9991706d8c2bd1773f35197912770360223a?"
              className="img-36"
              alt = ""
            />
          </div>
        </div>
        <div className="div-100">
          As a team license owner, I want to use multiplied
          limits
        </div>
        <div className="div-101">
          <div className="div-102">
            <img
              loading="lazy"
              src="https://cdn.builder.io/api/v1/image/assets/TEMP/0cf183460e563addb706582001b3d457613eec1c0f04b9605da7b761288a9c65?"
              className="img-37"
              alt = ""
            />
            <div className="div-103">21/03/22</div>
          </div>
          <div className="div-104">D-18</div>
        </div>
        <div className="div-105">
          <div className="div-106">
            <img
              loading="lazy"
              src="https://cdn.builder.io/api/v1/image/assets/TEMP/8e08ac811a5ff415fec8d93ec4f3aa879b040c1f410e139bdb743534532af8b5?"
              className="img-38"
              alt = ""
            />
            <div className="div-107">12 comments</div>
            <img
              loading="lazy"
              src="https://cdn.builder.io/api/v1/image/assets/TEMP/6ff1645f58ebf13109177cd47f8cab10cc2e685346d7308b162277b2c632d668?"
              className="img-39"
              alt = ""
            />
            <div className="div-108">0 files</div>
          </div>
          <img
            loading="lazy"
            srcSet="..."
            alt = ""
            className="img-40"
          />
        </div>
      </div>
      <div className="div-109">
        <img
          loading="lazy"
          srcSet="..."
          alt = ""
          className="img-41"
        />
      </div>
    </div>
  </div>
  <div className="column-5">
    <div className="div-110">
      <div className="div-111">
        <div className="div-112">In progress</div>
        <div className="div-113">
          <img
            loading="lazy"
            src="https://cdn.builder.io/api/v1/image/assets/TEMP/397670acc274a35abbdce1c95cebab78fccac6b31dfc024632ebf3788e687dd0?"
            className="img-42"
            alt = ""
          />
          <img
            loading="lazy"
            src="https://cdn.builder.io/api/v1/image/assets/TEMP/248d9e24c5c026a39aad4f37f34b1ec6c58a28c1263e3ea9bda2c06c438c8a4d?"
            className="img-43"
            alt = ""
          />
        </div>
      </div>
      <div className="div-114">
        <div className="div-115">
          <div className="div-116">
            Improve cards readability
          </div>
          <div className="div-117">
            <img
              loading="lazy"
              src="https://cdn.builder.io/api/v1/image/assets/TEMP/687f48c4aabd60ebf5bb8710157b9991706d8c2bd1773f35197912770360223a?"
              className="img-44"
              alt = ""
            />
          </div>
        </div>
        <p className="sample-text">
          As a team license owner, I want to use multiplied
          limits
        </p>
        <div className="div-118">
          <div className="div-119">
            <img
              loading="lazy"
              src="https://cdn.builder.io/api/v1/image/assets/TEMP/59a87043a64c1a19fc05196705727b4165ed40128e2196c7836a97d72fbb08f7?"
              className="img-46"
              alt = ""
            />
            <div className="div-120">21/03/22</div>
          </div>
          <div className="div-121">D-18</div>
        </div>
        <div className="div-122">
          <img
            loading="lazy"
            srcSet="..."
            alt = ""
            className="img-47"
          />
          <div className="div-123">
            <img
              loading="lazy"
              src="https://cdn.builder.io/api/v1/image/assets/TEMP/8e08ac811a5ff415fec8d93ec4f3aa879b040c1f410e139bdb743534532af8b5?"
              className="img-48"
              alt = ""
            />
            <div className="div-124">12 comments</div>
            <img
              loading="lazy"
              src="https://cdn.builder.io/api/v1/image/assets/TEMP/6ff1645f58ebf13109177cd47f8cab10cc2e685346d7308b162277b2c632d668?"
              className="img-49"
              alt = ""
            />
            <div className="div-125">0 files</div>
          </div>
        </div>
      </div>
      <div className="div-126">
        <div className="div-127">
          <div className="div-128">
            Improve cards readability
          </div>
          <div className="div-129">
            <img
              loading="lazy"
              src="https://cdn.builder.io/api/v1/image/assets/TEMP/687f48c4aabd60ebf5bb8710157b9991706d8c2bd1773f35197912770360223a?"
              className="img-50"
              alt = ""
            />
          </div>
        </div>
        <div className="div-130">
          As a team license owner, I want to use multiplied
          limits
        </div>
        <div className="div-131">
          <div className="div-132">
            <img
              loading="lazy"
              src="https://cdn.builder.io/api/v1/image/assets/TEMP/0cf183460e563addb706582001b3d457613eec1c0f04b9605da7b761288a9c65?"
              className="img-51"
              alt = ""
            />
            <div className="div-133">21/03/22</div>
          </div>
          <div className="div-134">D-18</div>
        </div>
        <div className="div-135">
          <div className="div-136">
            <img
              loading="lazy"
              src="https://cdn.builder.io/api/v1/image/assets/TEMP/8e08ac811a5ff415fec8d93ec4f3aa879b040c1f410e139bdb743534532af8b5?"
              className="img-52"
              alt = ""
            />
            <div className="div-137">12 comments</div>
            <img
              loading="lazy"
              src="https://cdn.builder.io/api/v1/image/assets/TEMP/6ff1645f58ebf13109177cd47f8cab10cc2e685346d7308b162277b2c632d668?"
              className="img-53"
              alt = ""
            />
            <div className="div-138">0 files</div>
          </div>
          <img
            loading="lazy"
            srcSet="..."
            alt = ""
            className="img-54"
          />
        </div>
      </div>
      <div className="div-139">
        <div className="div-140">
          <div className="div-141">
            <div className="div-142">D-3</div>
            <div className="div-143">Update tokens</div>
          </div>
          <div className="div-144">
            <img
              loading="lazy"
              src="https://cdn.builder.io/api/v1/image/assets/TEMP/e85eefe7dd11258fc45ee7eefedf85d5fd2946b10870fc61a827f726596d38ba?"
              className="img-55"
              alt = ""
            />
          </div>
        </div>
        <img
          loading="lazy"
          srcSet="..."
          alt = ""
          className="img-56"
        />
      </div>
    </div>
  </div>
</div>
</div>